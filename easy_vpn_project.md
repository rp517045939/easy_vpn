...
| 项目 | 详情 |
|------|------|
| IP / SSH | `119.91.235.65`，端口 `22`，用户 `ubuntu`，密钥 `~/.ssh/codex_migration` |
...
外部用户
  │
  ▼
nas.ruanpengpeng.cn / mac.ruanpengpeng.cn / vpn.ruanpengpeng.cn
  │
  ▼
Nginx（443，已有）── 新增 server block ──►  easy_vpn Server（本地端口，如 :8080）
                                                │
                                     WebSocket 隧道（WSS）
                                      ┌─────────┴──────────┐
                                      ▼                    ▼
...
选择 WebSocket 的理由：
- **直接复用现有 Nginx**，无需额外暴露端口
- 走 443 端口，穿透防火墙能力强
- 天然双向通信，适合隧道
- TLS 加密由 Nginx 处理，Server 侧代码不需要处理证书
- 对比 frp 的 TCP + yamux 方案，更容易与 Nginx 生态集成
...
├── server/
│   ├── main.py              # FastAPI 入口
│   ├── tunnel_manager.py    # 管理所有 Client 的 WebSocket 连接
│   ├── proxy.py             # 将外部 HTTP 请求路由到对应隧道
│   └── api.py               # 管理面板的 REST API（在线状态、流量统计等）
│
├── client/
│   ├── main.py              # 客户端入口，连接 Server，注册隧道规则
│   ├── forwarder.py         # 将隧道流量转发到本地端口
│   └── config.yml           # 客户端配置（server 地址、token、本地转发规则）
│
├── dashboard/               # Vue 3 管理面板
│   ├── src/
│   │   ├── views/
│   │   │   ├── Dashboard.vue   # 主面板：设备在线状态、隧道列表
│   │   │   └── Login.vue
│   │   └── api/             # 调用后端 REST API
│   └── vite.config.js
│
├── docker-compose.yml       # 服务端一键部署
├── config.example.yml       # 服务端配置示例
...
- [ ] 隧道多路复用（一条 WebSocket 连接上跑多个并发请求）
- [ ] Server proxy.py：收到外部 HTTP 请求，通过隧道转发给对应 Client
- [ ] Client forwarder.py：收到转发请求，连接本地服务，回传响应
...
- [ ] Windows 客户端系统托盘 + 开机自启
- [ ] 面板支持动态添加/删除隧道规则（不重启服务）
